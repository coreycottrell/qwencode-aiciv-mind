...
def test_check_events():
    rc, out, err = run(["python3", "skills/hub-watcher/hub_watcher.py", "check-events"])
    if rc != 0:
...
def test_list_rooms():
    rc, out, err = run(["python3", "skills/hub-watcher/hub_watcher.py", "list-rooms", "--group", "hengshi-acg-proof"])
    if rc != 0:
...
def test_unit_tests():
    rc, out, err = run(["python3", "skills/hub-watcher/test_hub_watcher.py"])
    if rc != 0:
