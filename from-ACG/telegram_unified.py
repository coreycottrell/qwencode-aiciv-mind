#!/usr/bin/env python3
"""
Unified Telegram Bot for AiCIV Civilization
...
- Auto-detects CIV tmux sessions
...
CIV_ID = os.environ.get("CIV_ID", _civ)
...
# Primary pane marker — written by session_pane_update hook on every PRIMARY session start.
# Holds just the pane ID (e.g. "%157"). TG bot reads this first, falls back to auto-detect.
# Stored in .tg_sessions/ so it survives independently of .current_pane.
_TG_SESSIONS_DIR = Path(__file__).parent.parent / ".tg_sessions"
PRIMARY_PANE_MARKER = _TG_SESSIONS_DIR / "primary_pane_id"
...
# Paths
PROJECT_ROOT = Path(__file__).parent.parent
CONFIG_FILE = Path(os.environ.get("TG_CONFIG_FILE", str(PROJECT_ROOT / "config" / "telegram_config.json")))
...
class TelegramBot:
    ...
    def __init__(self, config: Dict):
        ...
        # Tracks recent message hashes with timestamps — 60-second window only
        self._recent_injections: dict = {}  # {hash: timestamp}
    ...
    async def start(self):
        ...
        # Detect tmux session — respect explicit config overrides first
        config_pane = self.config.get("tmux_pane")
        config_session = self.config.get("tmux_session")
        if config_pane and not self.config.get("session_auto_detect", True):
            self.tmux_pane = config_pane
            self.tmux_session = config_session or config_pane.split(":")[0]
            logger.info(f"Using explicit tmux pane from config: {self.tmux_pane}")
        else:
            self.tmux_session = self._detect_acg_session()
            if self.tmux_session:
                # Pane resolution priority:
                #   1. PRIMARY_PANE_MARKER (.tg_sessions/primary_pane_id) — most authoritative
                #   2. .current_pane (legacy marker, written by hook and recovery logic)
                #   3. Auto-detect via _find_active_claude_pane() (title-aware scan)
                resolved = None
                for marker_path, label in [
                    (PRIMARY_PANE_MARKER, "primary_pane_id"),
                    (PROJECT_ROOT / ".current_pane", ".current_pane"),
                ]:
                    if marker_path.exists():
                        candidate = marker_path.read_text().strip()
                        if candidate and self._validate_pane(candidate):
                            resolved = candidate
                            logger.info(f"Startup: using pane {candidate!r} from {label}")
                            break
                        else:
                            logger.warning(f"Startup: {label} has stale pane {candidate!r}, trying next source...")

                if resolved:
                    self.tmux_pane = resolved
                else:
                    scanned = self._find_active_claude_pane()
                    self.tmux_pane = scanned or f"{self.tmux_session}:0.0"
                    if scanned:
                        logger.info(f"Startup: auto-detected pane {scanned}")
                    else:
                        logger.warning("Startup: could not detect pane, using session:0.0 fallback")
                logger.info(f"Detected CIV tmux session: {self.tmux_session}, pane: {self.tmux_pane}")
            else:
                logger.warning("No CIV tmux session detected - injection will fail")
        ...
        await self._send_message(f"Telegram bot online. Claude log streaming active.{mode_note}")
        ...
        try:
            while self._running:
                ...
                if now - last_session_refresh >= 60:
                    ...
                    # Heartbeat log so we can tell the bot is alive
                    logger.info(f"[heartbeat] alive | session={self.tmux_session} pane={self.tmux_pane}")
    ...
    async def _refresh_session_if_needed(self):
        """
        Check if a newer CIV session or pane exists and switch to it.

        Two-layer check (always-on TG→tmux fix):
          1. If .current_pane changed (written by session_pane_update hook), adopt it.
          2. Validate the active pane ID is still alive in tmux.
             If stale, scan live panes for the real active Claude pane.
        """
        try:
            auto_detect = self.config.get("session_auto_detect", True)
            ...
            if new_session and new_session != self.tmux_session:
                ...
                if not auto_detect:
                    pass  # Explicit pane configured — never override it
                else:
                    # SYSTEM fix: read primary_pane_id FIRST (authoritative, never hijacked by team leads)
                    # Fall back to .current_pane only if primary_pane_id is absent/stale
                    candidate = None
                    if PRIMARY_PANE_MARKER.exists():
                        candidate = PRIMARY_PANE_MARKER.read_text().strip()
                    if not candidate:
                        pane_marker = PROJECT_ROOT / ".current_pane"
                        candidate = pane_marker.read_text().strip() if pane_marker.exists() else None
                    if candidate and self._validate_pane(candidate):
                        self.tmux_pane = candidate
                    else:
                        # marker is stale or absent — scan live panes
                        scanned = self._find_active_claude_pane()
                        self.tmux_pane = scanned or f"{new_session}:0.0"
                await self._send_message(f"Switched to new session: {new_session}, pane: {self.tmux_pane}")

            elif new_session and new_session == self.tmux_session:
                # Same session — check if pane marker changed OR current pane went stale
                if not auto_detect:
                    pass  # Explicit pane configured — never override it
                else:
                    # SYSTEM fix: use primary_pane_id as authoritative source (immune to team lead hijacking)
                    # Only fall back to .current_pane if primary_pane_id is absent
                    marker_pane = None
                    if PRIMARY_PANE_MARKER.exists():
                        marker_pane = PRIMARY_PANE_MARKER.read_text().strip()
                    if not marker_pane:
                        pane_marker = PROJECT_ROOT / ".current_pane"
                        marker_pane = pane_marker.read_text().strip() if pane_marker.exists() else None
                    # --- Layer 1: adopt marker if it changed ---
                    if marker_pane and marker_pane != self.tmux_pane:
                        if self._validate_pane(marker_pane):
                            logger.info(f"Pane change via primary_pane_id: {self.tmux_pane} -> {marker_pane}")
                            self.tmux_pane = marker_pane
                        else:
                            logger.warning(f"Marker pane {marker_pane} is stale, scanning...")
                            scanned = self._find_active_claude_pane()
                            if scanned and scanned != self.tmux_pane:
                                logger.info(f"Pane corrected by scan: {self.tmux_pane} -> {scanned}")
                                self.tmux_pane = scanned

                    # --- Layer 2: validate the pane we are currently using ---
                    if self.tmux_pane and not self._validate_pane(self.tmux_pane):
                        logger.warning(f"Current pane {self.tmux_pane} is no longer alive. Scanning for active Claude pane...")
                        scanned = self._find_active_claude_pane()
                        if scanned:
                            logger.info(f"Self-corrected stale pane: {self.tmux_pane} -> {scanned}")
                            old_pane = self.tmux_pane
                            self.tmux_pane = scanned
                            await self._send_message(
                                f"Auto-corrected stale pane {old_pane} -> {scanned} (session {self.tmux_session})"
                            )
                        else:
                            logger.error(f"Could not find any live Claude pane. Injection will fail until next session.")
    ...
    async def _handle_command(self, text: str, user_id: str, username: str):
        ...
        if cmd == "/start" or cmd == "/help":
            ...
            help_text = f"""AiCIV Telegram Bridge
...
/resume - Pick a Claude session to follow
...
Last success: {uptime:.0f}s ago""" + (f"\nDowntime: {downtime:.0f}s" if downtime > 0 else "")
            ...
            parts = text.split()
            sessions = sorted(LOG_ROOT.glob("*.jsonl"), key=lambda p: p.stat().st_mtime, reverse=True)[:5]
            if not sessions:
                await self._send_message("No sessions found.")
                return
            if len(parts) == 1:
                # Show session list
                lines = ["Recent sessions (reply /resume N to switch):"]
                for i, s in enumerate(sessions, 1):
                    mtime = datetime.fromtimestamp(s.stat().st_mtime).strftime("%m-%d %H:%M")
                    active = " ← current" if s.stem == self.current_session else ""
                    lines.append(f"{i}. {s.stem[:20]}... ({mtime}){active}")
                await self._send_message("\n".join(lines))
            else:
                ...
                try:
                    ...
                    if 0 <= idx < len(sessions):
                        session_id = sessions[idx].stem
                        self._switch_session(session_id)
                        await self._send_message(f"Switched to: {session_id[:24]}...")
                    else:
                        await self._send_message(f"Pick 1-{len(sessions)}.")
                except ValueError:
                    await self._send_message("Usage: /resume 2")

        elif cmd == "/restart":
            await self._send_message("Restarting bot... will be back in a few seconds.")
            await asyncio.sleep(0.5)
            sys.exit(0)  # systemd/cron watchdog will restart
    ...
    async def _inject_and_respond(self, text: str, username: str, chat_id: int = 0, chat_type: str = "private", chat_title: str = ""):
        ...
        # Only block identical messages sent within the last 60 seconds
        msg_hash = hashlib.md5(f"{text}:{username}:{chat_id}".encode()).hexdigest()
        now_ts = time.time()
        # Purge stale entries (older than 60s)
        self._recent_injections = {h: t for h, t in self._recent_injections.items() if now_ts - t < 60}
        if msg_hash in self._recent_injections:
            logger.warning(f"Duplicate injection prevented (within 60s): {text[:30]}...")
            return
        self._recent_injections[msg_hash] = now_ts
        ...
        try:
            ...
            # Stale pane — try to recover by re-detecting the live pane (max 3 attempts)
            logger.warning(f"tmux injection failed ({type(e).__name__}): {e}. Attempting pane re-detection...")
            recovered = False
            for attempt in range(1, 4):
                new_pane = self._find_active_claude_pane()
                if new_pane and new_pane != self.tmux_pane:
                    logger.info(f"[retry {attempt}/3] Recovered pane: {self.tmux_pane} -> {new_pane}")
                    self.tmux_pane = new_pane
                    # Write recovered pane to BOTH markers so hook and future cycles pick it up
                    try:
                        (PROJECT_ROOT / ".current_pane").write_text(new_pane)
                        PRIMARY_PANE_MARKER.write_text(new_pane + "\n")
                    except IOError:
                        pass
                elif new_pane:
                    logger.info(f"[retry {attempt}/3] Same pane {new_pane}, retrying injection anyway")
                    self.tmux_pane = new_pane
                else:
                    logger.error(f"[retry {attempt}/3] No live Claude pane found")
                    break
                ...
                try:
                    ...
                    logger.info(f"tmux injection recovered on attempt {attempt}: {formatted[:50]}...")
                    await self._send_message(f"Injected to Claude (recovered pane {self.tmux_pane}). Awaiting response...")
                    recovered = True
                    break
                except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as retry_e:
                    logger.warning(f"[retry {attempt}/3] Injection still failed: {retry_e}")

            if not recovered:
                logger.error("tmux injection failed after 3 recovery attempts")
                await self._send_message(f"Error: tmux injection failed (pane {self.tmux_pane} unresponsive after 3 retries)")
    ...
    def _validate_pane(self, pane_id: str) -> bool:
        """
        Verify a pane ID (e.g. %105) actually exists in the current tmux server.
        Uses 'tmux list-panes -a' which lists ALL panes across all sessions.
        Returns True if the pane is alive, False if stale.
        """
        if not pane_id:
            return False
        try:
            ...
            live_panes = result.stdout.strip().splitlines()
            return pane_id in live_panes
        except Exception:
            return False

    def _find_active_claude_pane(self) -> Optional[str]:
        """
        Scan all tmux panes and return the ID of the PRIMARY Claude pane.

        Priority order (highest to lowest):
          1. PRIMARY_PANE_MARKER file (.tg_sessions/primary_pane_id) — written by hook
          2. Pane titled "Claude Code" in the most recent acg-primary-* session
          3. Most recently used pane in any acg-primary-* session (legacy fallback)

        Returns pane_id string like '%157', or None if not found.
        """
        # --- Priority 1: primary pane marker file ---
        try:
            if PRIMARY_PANE_MARKER.exists():
                candidate = PRIMARY_PANE_MARKER.read_text().strip()
                if candidate and self._validate_pane(candidate):
                    logger.info(f"[pane-scan] Using PRIMARY_PANE_MARKER: {candidate}")
                    return candidate
                else:
                    logger.warning(f"[pane-scan] PRIMARY_PANE_MARKER {candidate!r} is stale, scanning...")
        except Exception:
            pass
        ...
        try:
            result = subprocess.run(
                ["tmux", "list-panes", "-a",
                 "-F", "#{pane_id} #{session_name} #{pane_title} #{pane_last_used}"],
                capture_output=True, text=True, timeout=5
            )
            candidates = []
            for line in result.stdout.strip().splitlines():
                ...
                # pane_title may contain spaces — last token is always pane_last_used (epoch int)
                try:
                    last_used = int(parts[-1])
                    pane_title = " ".join(parts[2:-1])
                except ValueError:
                    last_used = 0
                    pane_title = " ".join(parts[2:])
                if session_name.startswith(f"{CIV_ID}-primary-"):
                    # Score: 2 = titled "Claude Code" (primary pane), 1 = other ACG pane
                    title_score = 2 if "Claude Code" in pane_title else 1
                    candidates.append((title_score, last_used, pane_id, session_name))

            if not candidates:
                return None

            # Sort: title_score DESC (Claude Code first), then last_used DESC (most recent)
            candidates.sort(key=lambda x: (x[0], x[1]), reverse=True)
            best_pane = candidates[0][2]
            best_session = candidates[0][3]
            logger.info(f"[pane-scan] Found active Claude pane: {best_pane} in {best_session} (title_score={candidates[0][0]})")
            return best_pane

        except Exception as e:
            logger.error(f"[pane-scan] Error scanning panes: {e}")
            return None
...
async def main():
    ...
    logger.info("Starting AiCIV Unified Telegram Bot")
